...
function HeroOpportunity({ opp }: { opp: Opportunity }) {
  ...
  return (
    <div className="relative bg-white/[0.03] backdrop-blur-xl border border-white/12 rounded-3xl p-8 md:p-10 overflow-hidden mb-10">
      {/* Glow */}
      <div className="absolute inset-0 bg-gradient-to-br from-blue-600/6 via-transparent to-violet-600/6 pointer-events-none" />
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-4 mb-4">
              <div className="w-14 h-14 flex items-center justify-center bg-white/10 border border-white/12 rounded-2xl shrink-0">
                <span className="text-sm font-bold text-white">{opp.symbol.slice(0, 3)}</span>
              </div>
              <div>
                <h2 className="text-2xl font-bold text-white">{opp.companyName}</h2>
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-zinc-400 text-sm font-mono">{opp.symbol}</span>
                  <span className="text-zinc-600">·</span>
                  <span className="text-zinc-400 text-sm">{opp.sector}</span>
                </div>
              </div>
            </div>

            <div className="flex items-end gap-4 mb-5">
              <span className="text-4xl font-bold text-white tabular-nums">{opp.price}</span>
              <span className={`text-xl font-bold tabular-nums mb-1 ${opp.isPositive ? "text-green-400" : "text-red-400"}`}>
                {opp.changePercent}
              </span>
            </div>

            <p className="text-zinc-300 leading-relaxed text-sm mb-5 max-w-lg">{opp.explanation}</p>
            ...
            {/* Catalyst */}
            <div className="flex items-start gap-2.5 p-3 bg-white/[0.04] border border-white/8 rounded-xl">
              <span className="text-sm shrink-0">⚡</span>
              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mb-0.5">Latest Catalyst</p>
                <p className="text-sm text-zinc-300">{opp.latestCatalyst}</p>
              </div>
            </div>
          </div>
          ...
          <div className="w-full lg:w-72 shrink-0">
            ...
            <div className="mb-5">
              ...
              <div className="h-2 bg-white/8 rounded-full overflow-hidden">
                <div className="h-full bg-gradient-to-r from-blue-400 to-violet-400 rounded-full" style={{ width: `${opp.confidence}%` }} />
              </div>
            </div>
            ...
            <Link
              ...
              className="
                ...
                bg-white text-black px-5 py-3 rounded-full text-sm font-bold
                hover:bg-zinc-100 transition-all duration-150 hover:-translate-y-0.5
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <h1 className="text-3xl font-bold text-white tracking-tight">Opportunity Radar</h1>
            </div>
            <p className="text-zinc-400 text-sm leading-relaxed max-w-lg">
              AlphaVerse proactively scores every company using live financial data, news sentiment,
              and technical signals — so you never miss an opportunity.
            </p>
          </div>
